...
class CustomerContactController extends Controller
{
    ...
    public function addBusinessContact(Request $request, $customer_id, $business_id)
    {
        $validated = $request->validate([
            ...
            'contact_person_type'  => 'nullable|string|max:100',
        ...
        $business = CustomerBusinessProfile::where('customer_id', $customer_id)->where('id', $business_id)->first();
        ...
        if ($isPrimary && in_array($validated['contact_type'], ['phone', 'email'])) {
            $updateData = [];
            if ($validated['contact_type'] === 'email') $updateData['primary_email'] = $validated['value'];
            if ($validated['contact_type'] === 'phone') $updateData['primary_phone'] = ($validated['country_code'] ?? '') . $validated['value'];
    ...
    public function updateBusinessContact(Request $request, $customer_id, $business_id, $contact_id)
    {
        ...
        $contact = CustomerContact::where('customer_id', $customer_id)->where('business_id', $business_id)->find($contact_id);
        ...
        if (in_array($contactType, ['phone', 'email'])) {
            ...
            if ($isPrimaryNow) {
                ...
                $hasOther = CustomerContact::where('customer_id', $customer_id)
                    ->where('business_id', $business_id)
                    ->where('contact_type', $contactType)
                    ->where('id', '!=', $contact_id)
                    ->where('is_primary', true)
